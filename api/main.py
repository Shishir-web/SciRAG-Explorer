...
from api.dependencies import limiter, check_db_health
...
app = FastAPI(
    ...
    description = "Multi-agent scientifc literature RAG system",
    version     = "1.0.0",
    docs_url    = "/docs",
    redoc_url   = "/redoc"
)
...
def format_response(state: dict) -> QueryResponse:
    ...
    return QueryResponse(
        ...
        retrieval_passes  = state.get("retrieval_feedback", 0),
        conflicts         = conflicts,
        grounding         = grounding,
        has_conlicts      = len(conflicts) > 0,
    )
            ...
            yield {
                ...
                "data":   json.dumps({
                    "message":(
                        ...
                        f"deteced {len(state.get('conflicts', []))} conflicts"
                    )
                }),
            }
